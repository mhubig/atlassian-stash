...
psql_dump_cmd = 'docker run -it --rm --link stash_database_1:db -v $(pwd):/tmp\
 postgres sh -c \'pg_dump -U stash -h "$DB_PORT_5432_TCP_ADDR" \
-w stash > /tmp/stash.dump\''

bup_backup_cmd = 'BUP_DIR=$(pwd)/backup bup save -n stashbackup $(pwd)'

bup_restore_cmd = 'BUP_DIR=$(pwd)/backup bup restore -C stashbackup \
